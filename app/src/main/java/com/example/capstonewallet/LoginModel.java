...
public class LoginModel {
    ...
    public void loginAccount(LoginViewModel model, AccountRepository repository) {
        Log.d("yo123", publicKey);
        Log.d("yo123", password);
        if(repository.getAccountFile(model.getPublicKey()) == null) {
            Log.d("yo123", "inserted");
            repository.insertAccount(model.getPublicKey(), model.getPassword());
        }
        //repository.insertAccount(publicKey.getText().toString(), password.getText().toString());
        String file = repository.getAccountFile(model.getPublicKey());
